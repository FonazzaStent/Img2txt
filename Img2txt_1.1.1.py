...
def encode(imgfilename):
    ...
    img3=im2.convert("L", palette=Image.Palette.ADAPTIVE, dither=Image.Dither.FLOYDSTEINBERG, colors=256)
    ...
    for pixels in range (1,imgfile_size):
        bytea=image2.read(1)
        bytevalue=int.from_bytes(bytea, "big")
        char=chr(int(bytevalue/9.8)+97)
